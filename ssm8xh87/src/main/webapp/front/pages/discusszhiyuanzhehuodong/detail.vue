...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusszhiyuanzhehuodongCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusszhiyuanzhehuodongCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusszhiyuanzhehuodong');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusszhiyuanzhehuodong');
			...
			async init(){
				...
				let res = await this.$api.info('discusszhiyuanzhehuodong', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusszhiyuanzhehuodong', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusszhiyuanzhehuodong/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
