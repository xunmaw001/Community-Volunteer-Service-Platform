	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusshuodongfengcai`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusshuodongfengcai`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusshuodongfengcai`, this.ruleForm);
						}else{
							await this.$api.add(`discusshuodongfengcai`, this.ruleForm);
						}
						uni.setStorageSync('discusshuodongfengcaiCleanType',true);
