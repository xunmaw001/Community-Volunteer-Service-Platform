...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshuodongfengcaiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusshuodongfengcaiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusshuodongfengcai');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusshuodongfengcai');
			...
			async init(){
				...
				let res = await this.$api.info('discusshuodongfengcai', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshuodongfengcai', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshuodongfengcai/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
