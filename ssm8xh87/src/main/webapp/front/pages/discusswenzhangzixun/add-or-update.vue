	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusswenzhangzixun`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusswenzhangzixun`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusswenzhangzixun`, this.ruleForm);
						}else{
							await this.$api.add(`discusswenzhangzixun`, this.ruleForm);
						}
						uni.setStorageSync('discusswenzhangzixunCleanType',true);
