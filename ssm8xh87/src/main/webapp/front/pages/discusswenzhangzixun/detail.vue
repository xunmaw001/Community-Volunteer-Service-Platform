...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusswenzhangzixunCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusswenzhangzixunCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusswenzhangzixun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusswenzhangzixun');
			...
			async init(){
				...
				let res = await this.$api.info('discusswenzhangzixun', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusswenzhangzixun', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusswenzhangzixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
