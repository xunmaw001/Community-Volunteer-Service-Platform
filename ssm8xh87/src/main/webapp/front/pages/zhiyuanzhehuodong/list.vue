		...
		<view :style='{"width":"100%","padding":"0 0 64rpx","position":"relative","background":"url(http://codegen.caihongy.cn/20221108/581bb1956ffe4c8182127ca1e196e95e.png) fixed","height":"auto"}'>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","borderRadius":"8rpx 8rpx 0 0","background":"#fff","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>活动地点:{{product.huodongdidian}}</view>
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","borderRadius":"8rpx 8rpx 0 0","background":"#fff","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>活动时间:{{product.huodongshijian}}</view>
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zhiyuanzhehuodong','修改')) || (!userid && isAuthFront('zhiyuanzhehuodong','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zhiyuanzhehuodong','删除')) || (!userid && isAuthFront('zhiyuanzhehuodong','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
        ...
        params['order'] = 'desc';
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zhiyuanzhehuodong`, params);
                } else {
                    res = await this.$api.list(`zhiyuanzhehuodong`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhiyuanzhehuodong', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zhiyuanzhehuodong`, searchForm);
                } else {
                    res = await this.$api.list(`zhiyuanzhehuodong`, searchForm);
                }
				// 如果是第一页数据置空
