	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230330/2fc70680a5454e95907c7c0d811bb812.jpg) fixed no-repeat center top /100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="风采内容" prop="fengcaineirong">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.fengcaineirong" label="风采内容" prop="fengcaineirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.fengcaineirong"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				fengcaineirong: [
					{ required: true, message: '风采内容不能为空', trigger: 'blur' },
				],
				fabushijian: [
				],
				shequzhanghao: [
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.fengcaineirong = this.ruleForm.fengcaineirong.replace(reg,'../../../ssm8xh87/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huodongfengcai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huodongfengcaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
